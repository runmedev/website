...
interface TitleExpandDetailsProps {
  title: string;
  description: string;
  upcoming?: boolean;
  id: string;
...
const TitleExpandDetails: React.FC<TitleExpandDetailsProps> = ({ title, description, upcoming, id }) => {
...
export interface Show {
  ...
  host?: string[];
  hostAvatars?: string[];
  postShowLinks?: string[];
  eventAssets: string[];
}

interface RoadShowUIProps {
  shows: Show[];
}

const RoadShowUI: React.FC<RoadShowUIProps> = ({ shows }) => {
  useEffect(() => {
    AOS.init();
  }, []);
        ...
        {shows.map((show, i) => {
          return (
            <div id={show.id} className="relative" key={i}>
              ...
              >
                ...
                <div className="absolute z-20 flex items-center justify-center w-full h-full mx-auto ">
                  <div className="flex flex-col text-center md:space-y-2 lg:space-y-6">
                    ...
                    {show.postShowLinks && show.postShowLinks.length > 0 && (
                      <div className="lg:text-xl text-white text-center">
                        Show Notes:{" "}
                        {show.postShowLinks.map((link, j) => (
                          <Link href={link} className="underline" key={j}>
                            {link}
                          </Link>
                        ))}
                      </div>
              ...
              >
                {show.eventAssets.map((picture, j) => (
