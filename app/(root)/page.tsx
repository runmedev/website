...
const communityMap: Record<string, CommunityEntry> = {
  star: {
...
const LinkWrap = ({ href, alt, children }: LinkWrapProps) => {
  return (
...
const CommunityBox = ({ entry }: CommunityBoxProps) => {
...
const Hero = () => {
  return (
    <div className="flex flex-col space-y-3 text-center px-4 md:px-0">
      <a
        href="https://visr.dev/?utm_source=runme.dev&utm_medium=banner&utm_campaign=visr-launch&utm_content=claude-md-untested"
        target="_blank"
      ...
      >
        <div className="flex flex-wrap items-center justify-center gap-2 rounded-full border border-yellow-400 bg-neutral-950/80 px-3 py-2 text-sm text-gray-200 shadow-lg shadow-yellow-400/30 hover:bg-neutral-900 md:px-4 md:text-base">
          ...
          <span>Your CLAUDE.md is untested code. →</span>
        </div>
      </a>
...
const GuideEntry = ({ title, description, url, iconPath }: GuideEntryProps) => {
  return (
...
const TutorialEntry = ({ title, description, url, imgSrc }: TutorialEntryProps) => {
  return (
