...
const ShowEntry = ({ entry }: ShowEntryProps) => {
...
const Examples = () => {
  const [entries, setEntries] = useState<ExampleEntry[]>(examplesArr as ExampleEntry[]);
  const tags = [...new Set((examplesArr as ExampleEntry[]).map((entry) => entry.tags).flat())];
  const [tagsOn, setTagsOn] = useState<string[]>([]);
  const [filter, setFilter] = useState<string>("");

  useEffect(() => {
    setEntries(
      (examplesArr as ExampleEntry[])
        .filter((entry) => {
